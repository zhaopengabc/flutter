...
class HttpUtil {
  ...
  HttpUtil(context) {
    ...
    _dio.interceptors.add(InterceptorsWrapper(onRequest: (RequestOptions options) {
      ...
      if (e.message.contains('Network is unreachable')) {
        if (!checkCode(codeList, Code.NETWORK_UNREACHABLE)) { // 50000
          msgErr = '网络未连接，请检查网络设置';
        } else {
          msgErr = null;
        }
      } else if (e.message.contains('No route to host')) {
        if (!checkCode(codeList, Code.NETWORK_NO_HOST)) { // 50001
          msgErr = '操作失败，请检查设备是否在网络中';
        } else {
          msgErr = null;
        }     
      } else if (e.message.contains('Connecting timed out')) {
        if (!checkCode(codeList, Code.NETWORK_TIMEOUT)) { // 50002
  ...
  Future<Result> request(url, {data, method, cancelToken, sendTimeout = 0, List<int> passCodeList}) async {
    // print('请求的URL11111---$url-----${_instance?.baseOptions?.baseUrl}');
    try {
      data = data ?? {};
      ...
      Response response;
      try {
        response = await _dio.request(url, data: data, options: _options, cancelToken: cancelToken);
        // print('服务返回-----${response.data["status"]}');
      } on DioError catch (e) {
      ...
      if (passCodeList.length == 0 && response.data['status'] != 0) {
        String nonZeroMsg;
        // 特殊处理状态
        if ((response.data['status'] > 105 && response.data['status'] < 109 && _isLocal) || (response.data['status'] == 101 && _isLocal)) {
        ...
        if(null == nonZeroMsg && null != Translations.of(_context).text("message.http${response.data["status"]}")) {
          Fluttertoast.showToast(
            msg: '${Translations.of(_context).text("message.http${response.data["status"]}")}',
            toastLength: Toast.LENGTH_SHORT,
            backgroundColor: ColorMap.toast_error_color,
            textColor: ColorMap.white,
            gravity: ToastGravity.TOP);
      ...
      return new Result(response.data['status'], response.data['msg'], response.data['data']); // 返回业务数据
    } catch (e) {
      codeList = List();
      Fluttertoast.showToast(
  ...
  void requestCallback(Function funCallback, url,
      {data, method, cancelToken, List<int> passCodeList, whenErrorIsCallback = false}) async {
    Result result =
        await request(url, data: data, method: method, cancelToken: cancelToken, passCodeList: passCodeList);
    if (result.code != 0) {
  ...
  handlerError(DioError e) {
    ...
    if (e.response != null) {
      ...
      errorResponse = new Response(statusCode: 888); // 888:网络请求错误处理
    }
