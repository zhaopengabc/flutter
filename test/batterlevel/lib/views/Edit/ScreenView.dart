...
class _ScreenViewState extends State<ScreenView>  with TickerProviderStateMixin, WidgetsBindingObserver{
  ...
  static const double layerHeaderHeight = 56; //图层header高度
  static const double layerMinWidth = 64;
  static const double layerMinHeight = 64;

  double _preOffsetX = 0;
  double _preOffsetY = 0;
  int _draggedLayerId = -1;
      ...
      child: CustomerSizeChangedLayoutNotifier(
        child: GestureDetector(
          child: Container(
            ...
            color: Colors.transparent,//ColorMap.screen_bkg_color,//
            ...
            Stack(
              ...
              children: <Widget>[
                Container(
                  ...
                  width: Utils.setWidth(LayoutConstant.EditWidth),
                  // height: Utils.setHeight(LayoutConstant.EditHeight),
                  padding: EdgeInsets.fromLTRB(0, Utils.setHeight(24), 0, Utils.setHeight(64)),
                  alignment: Alignment.center,
                  child: 
...
//图层stack
  Widget layerListWidget() {
    ...
    for (var item in customerLayers) {
      ...
      layers.add(new Position(
        layerId: item.screenLayer.layerId,
        left: item.layerPosition.left,
        top: item.layerPosition.top,
      ...
      layers.add(new Position(
        layerId: item.screenLayer.layerId,
        left: item.layerPosition.left,
        top: item.layerPosition.top + item.layerPosition.height,
        width: item.layerPosition.width,
        height: item.layerPosition.height
      ));
      layers.add(new Position(
        layerId: item.screenLayer.layerId,
        left: item.layerPosition.left + item.layerPosition.width,
        top: item.layerPosition.top,
        width: item.layerPosition.width,
        height: item.layerPosition.height
      ));
      ...
      // print('列表中的位置-----${item.screenLayer.layerId}------${item.layerPosition.left}----------${item.layerPosition.top}');
    }
    ...
    Provider.of<RenderBoxProvider>(context,listen:false).layerPositionList = layers;
    return content;
  }
...
//图层（不包括关闭、全屏按钮）
  Widget layerWidget(CustomerLayer item){
    ...
    double borderWidth = Provider.of<EditProvider>(context, listen: false).flickerLayerId == item.screenLayer.layerId ? Utils.setWidth(2.0):Utils.setWidth(1.0);
    ...
    return GestureDetector(
                        ...
                        Consumer<VideoState>(builder: (BuildContext context, data, Widget child) {
                          ...
                          return Container(
                            ...
                            child: (!layerSource) ?  Container(color: item.noSourceColor) :
                              ...
                              : isVideoClosed ?  Container(color: item.noSourceColor)//Image.asset('assets/images/input/input1.png', fit: BoxFit.fill)
                              : Container(
                        ...
                        Column(children: <Widget>[
                          ...
                          Container(
                            ...
                            child: Consumer<VideoState>(builder: (BuildContext context, data, Widget child) {
                              bool isVideoClosed = data.isVideoClosed == null? false : data.isVideoClosed;
                              return layerHeader(item, layerSource, noLayerSignal, borderWidth, isVideoClosed);
                            })
                          ),
                          //图层是否有信号
                          Expanded(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border:
                            Border.all(
                              ...
                              : Provider.of<EditProvider>(context, listen: false).selectedLayerId == item.screenLayer.layerId? ColorMap.border_layer_color : ColorMap.screen_color, //ColorMap.layer_border_color,
                            ),
                      ),
                    )
                  ],
            ...
            onScaleStart: (ScaleStartDetails e) {
              ...
              if(_draggedLayerId != -1) return;
              _draggedLayerId = item.screenLayer.layerId;
              Provider.of<EditProvider>(context, listen: false).selectedLayerId = item.screenLayer.layerId;
              Offset screenRenderBoxOffset = Provider.of<RenderBoxProvider>(context, listen: false).screenRenderBoxOffset;
              // LayerProvider layerProvider = Provider.of<LayerProvider>(context, listen: false);
              ...
              _preWidth = item.layerPosition.width;
              _preHeight = item.layerPosition.height;
              _preOffsetX = item.layerPosition.left;
              _preOffsetY = item.layerPosition.top;
              ...
              var currLayer = layers.where((p)=>p.layerId == item.screenLayer.layerId).toList();
              if(currLayer != null && currLayer.length > 0){
                for(int i = 0; i < currLayer.length;i++){
                  layers.remove(currLayer[i]);
                }
              }
              outoutcards.addAll(layers);
              LayerJustification.compareList = outoutcards;
              LayerJustification.clearPoints();
            },
            onScaleUpdate: (ScaleUpdateDetails e) {
              // print('移动更新${e.focalPoint.dx}');
              if(_draggedLayerId != item.screenLayer.layerId) return;
              Offset screenRenderBoxOffset = Provider.of<RenderBoxProvider>(context, listen: false).screenRenderBoxOffset;
              LayerProvider layerProvider = Provider.of<LayerProvider>(context, listen: false);
              num h = _preHeight;
              num w =  _preWidth;
              double left = e.focalPoint.dx - xDis - screenRenderBoxOffset.dx;
              double top = e.focalPoint.dy - yDis - screenRenderBoxOffset.dy;
              AdsorbedPosition adsorbedPosition = LayerJustification.adsorb(context, item, left, top, _preOffsetX, _preOffsetY);
              if(adsorbedPosition != null){
                if(adsorbedPosition.horizontal!=null){
                  left = adsorbedPosition.horizontal;
                }
                if(adsorbedPosition.vertical!=null){
                  top = adsorbedPosition.vertical;
                }
                if(adsorbedPosition.moveDirection == MoveDirection.none){
                  return;
                }
              }
              _preOffsetX = left;
  ...
  //图层标题、全屏按钮、删除按钮
  Widget layerHeader(CustomerLayer item, bool layerSource, bool noLayerSignal, double borderWidth, bool isVideoClosed){
    ...
    return Offstage(
      offstage: !layerSource || noLayerSignal || Provider.of<EditProvider>(context, listen: false).selectedLayerId == item.screenLayer.layerId || isVideoClosed? false : true,
      child:  Container(
        width: item.layerPosition.width - borderWidth,
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            Container(
              ...
              height: Utils.setWidth(layerHeaderHeight-18),
              alignment: Alignment.center,
              decoration: BoxDecoration(
  ...
  layerZoom(CustomerLayer customerLayer) {
    ...
    bool isOver = ResourceCalc.calcResource(editProvider.screenList, editProvider.currentScreenId, [{'id': customerLayer.screenLayer?.layerId, 'x': x, 'y': y, 'w': w, 'h': h,}]);
    if(isOver) {
...
//输入拼接stack
  Widget outputModeListWidget(List<ScreenInterface> screenInterfaces) {
    ...
    List<Widget> tiles = []; //先建一个数组用于存放循环生成的widget
    Widget content; //单独一个widget组件，用于返回需要生成的内容widget
    List<Position> outputCards = new List();
    for (var item in screenInterfaces) {
      if(item.x == 0 || item.y == 0) continue;
      double left = Utils.setWidth((item.x - screenRealX) / screenRealW * screenWidth);
      double top = Utils.setHeight((item.y - screenRealY) / screenRealH * screenHeight);
      num width = Utils.setWidth(item.width / screenRealW * screenWidth);
      num height = Utils.setHeight(item.height / screenRealH * screenHeight);
      tiles.add(Positioned(
        ...
        child: Container(
          ...
          height: height,
          decoration: BoxDecoration(
            color: bkgEnable == 1 ? ColorMap.screen_color_transparent : ColorMap.screen_color,
            border: Border.all(
    ...
    Provider.of<RenderBoxProvider>(context,listen:false).outputCardPositionList = outputCards;
    return content;
  }
...
//背景Stack
  ...
  List<CustomerLayer> generateLayersData(
    ...
    for (var item in layerListData) {
      ...
      if(item.source != null) {
        //中间件返回的sourceType为1，去查询输入列表，如果能查到为有源，如果查不到为无源 // s4: 查询输入源列表能查到并且online==1，才是有源图层
  ...
  fullScreen(CustomerLayer customerLayer) async{
    ...
    List layerChangeAfter = [{
      'id': customerLayer.screenLayer?.layerId,
      'x': x,
      'y': y,
      'w': w,
      'h': h
    }];
    if(!ResourceCalc.calcResource(editProvider.screenList, customerLayer.screenLayer?.screenId, layerChangeAfter)) {
      // print('变化后大小-${customerLayer.layerPosition.width}---${customerLayer.layerPosition.height}-- ${w}---${h}---${customerLayer.screenLayer?.window?.width}---${customerLayer.screenLayer?.window?.height}');
      Map layerParams = {
        'deviceId': 0,
        'screenId': customerLayer.screenLayer?.screenId,
